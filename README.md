...
Projeto **prático** (sem teoria — isso já é coberto em outros repositórios do
currículo) para aprender, na prática, os conceitos de sistemas distribuídos por
trás de um sistema de notificações (push/email/SMS) capaz de escalar para
milhões de usuários.

Baseado no artigo [Designing a Notification System That Can Handle 10 Million
Users](https://medium.com/@gaddamnaveen192/designing-a-notification-system-that-can-handle-10-million-users-cad6b4970b78)
(Gaddam Naveen).

Sem credenciais reais de Firebase/Twilio/SendGrid — cada provider externo é
simulado por um mock que reproduz o contrato e os erros reais do serviço, só
para servir de aprendizado.

Cada conceito abaixo vira código rodando de verdade + um teste automatizado
que comprova o comportamento real (Kafka, Redis e MySQL reais via
Testcontainers, não mocks de unidade).
...
- Chave de partição: `userId`, em todos os tópicos acima — garante que
  ...
  de cada grupo.
- Por worker, tópicos próprios de retry/DLQ: `notification.push.retry`,
...
Sem particionamento físico por data (fora de escopo, ver seção 1).

## 8. Providers mockados
...
- **`TwilioSmsProvider` (mock, em `sms-worker`)** — simula Twilio: número em
  formato inválido → `FAILED_NON_RETRYABLE`; usa o `RateLimiter` da seção 5.
...
_Em construção — instruções de `docker-compose` e execução dos testes com
Testcontainers serão adicionadas junto com o plano de implementação._
